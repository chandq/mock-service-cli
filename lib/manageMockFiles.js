 ...
 * @LastEditors: chendq
 * @LastEditTime: 2021-12-31 21:27:16
 * @Author: chendq
 */
...
const { getFileLatestContent, SupportMethods, filePath2ApiUrl, logger, getDataType, writeStream } = require('./utils');
...
const genMockFiles = function ({ url: apiUrl, method, data: resJsonData, dir: mockDir }) {
  ...
  if (fsa.pathExistsSync(mockFilePath)) {
    ...
    Object.keys(mockFileContent).length > 0 && writeStream(mockFilePath, JSON.stringify(mockFileContent, null, 2));
  } else {
