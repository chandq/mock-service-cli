 ...
 * @LastEditors: chendq
 * @LastEditTime: 2021-12-24 13:15:26
 * @Author: chendq
 */
...
const genMockFiles = function (apiUrl, method, resJsonData, mockDir) {
  ...
  if (fsa.pathExistsSync(mockListFilePath)) {
    ...
    mockApiMap = _.cloneDeep(oldMockListContent);
  } else {
  ...
  if (fsa.pathExistsSync(mockFilePath)) {
    ...
    if (oldFile.hasOwnProperty(method) && _.isEqual(oldFile[method], resJsonData)) {
      console.assert(
        !_.isEqual(oldFile[method], resJsonData),
    ...
    fsa.writeFileSync(mockFilePath, JSON.stringify(oldFile, null, 2));
  } else {
    if (!mockApiMap.hasOwnProperty(normalizeUrl)) {
      mockApiMap[normalizeUrl] = [method];
    } else if (!mockApiMap[normalizeUrl].includes(method)) {
  ...
  if (!oldMockListContent || !_.isEqual(oldMockListContent, mockApiMap)) {
    fsa.writeFileSync(mockListFilePath, JSON.stringify(mockApiMap, null, 2));
  }
  // console.debug('file::', `${mockDir}/${normalizeUrl}`, mockApiMap);
};
/**
