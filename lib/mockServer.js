...
const composeRoute = function (file) {
  ...
  console.info(colors.bgGreen('Mockfile: '), file);
  Object.keys(fileObject).forEach(item => {
    console.info(colors.bgYellow('Traversal url: '), item, colors.blue(typeof fileObject[item]));
    if (typeof fileObject[item] === 'function') {
      app.use(item, fileObject[item]);
      // console.info('function proxy: ', item)
    } else if (typeof fileObject[item] === 'object') {
      app.use(item, function (req, res) {
        res.json(fileObject[item]);
      });
      // console.info('plain object proxy: ', item)
    }
  });
};
/**
...
const parseMockFiles = function (specialDir = '../mock') {
  ...
  console.info(chalk.hex('#ce5b34bd')(`${files.length} mock file parsed`));
};
app.use(crossDomain());
...
app.listen(Number.parseInt(process.env.PORT, 10), () => {
  console.info(
    [
      colors.yellow('\nStarting up mock-server, serving '),
...
process.on('SIGINT', function () {
  console.info(colors.red('mock-server process stopped.'));
  process.exit();
});
