...
const { genMockFiles } = require('./manageMockFiles');
...
if (process.env.SOCKETSERVER) {
  ...
  socketServer.of('/write-mock-data').on('connection', function (socket) {
    console.log(colors.bgBlue(`Socket server has started. path: /ws/mock-service, `), socket);
    console.log(colors.blue(`event list: save-data`));
